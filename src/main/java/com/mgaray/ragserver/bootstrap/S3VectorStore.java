...
/*
    private void queryVectors(
            S3VectorsClient client,
            List<Float> queryEmbedding
    ) {
        // Simple metadata equality filter:
        // {"tenantId": "tenant-123"}
        Document filter = Document.mapBuilder()
                .putString("tenantId", "tenant-123")
                .build();
        ...
        QueryVectorsResponse response = client.queryVectors(request);

        System.out.println("Distance metric: " + response.distanceMetric());

        for (QueryOutputVector match : response.vectors()) {
            Object metadata = match.metadata() == null
                    ? null
                    : match.metadata().unwrap();

            System.out.printf(
                    "key=%s, distance=%s, metadata=%s%n",
                    match.key(),
                    match.distance(),
                    metadata
            );
